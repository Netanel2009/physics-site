...
let isSignUpMode = false; // האם אנחנו במצב הרשמה או התחברות

// פונקציה לפתיחה/סגירה של חלון המודאל
function toggleModal(show) {
    const modal = document.getElementById('login-modal');
    if (modal) modal.style.display = show ? 'flex' : 'none';
}

// מעבר בין "התחברות" ל"הרשמה" בתוך החלון
function toggleAuthMode() {
    isSignUpMode = !isSignUpMode;
    document.getElementById('modal-title').innerText = isSignUpMode ? 'הרשמה' : 'התחברות';
    document.getElementById('toggle-text').innerText = isSignUpMode ? 'כבר יש לך חשבון? התחבר' : 'אין לך חשבון? הירשם';
}

// ביצוע ההתחברות/הרשמה מול Firebase
...
// האזנה למצב המשתמש (האם הוא מחובר כרגע?)
firebase.auth().onAuthStateChanged(user => {
    ...
    if (user) {
        ...
        if (displayName) displayName.innerText = user.email.split('@')[0];
    } else {
